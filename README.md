...
初めに tkinter のウィンドウ`root`を定義し、これを`mainloop()`メソッドで起動させています。
ウィンドウ`root`のアトリビュートとして、以下の 2 つは最初に定義されることが多いです。
...
実は今回の目的である GUI 操作は、先ほど定義したウィンドウではなくフレームというものに追加することで初めて実現することができます。
