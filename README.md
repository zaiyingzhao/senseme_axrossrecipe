...
はじめに相手の疲労度を反映させるアバターを取得しましょう。今回は[イラストサイト素材サイト「ぴよたそ」](https://hiyokoyarou.com/)さんの gif 画像を利用させていただきます。
...
tkinterは通常ythonに含まれていますが、念のため以下のようにインストールが行われているか確認をしておきましょう。
...
実は今回の目的であるGUI操作は、ウィンドウではなくフレーム内に追加することで初めて実現できます。   
そこで、続いてウィンドウの中にフレームというものを定義します。   
...
これから追加していくGUI機能は、このmain_frameに追加していくことになります。   

今回はボタンとラベル（文字の設定）を追加してみましょう。
...
`tk.Button()`の第1引数には先ほどのフレームを指定します。   
...
先ほど実装したボタンには"change avatar"という文字を記入し、またその文字のフォントやサイズを指定していますが、これが表示されるためには上のコードのようにラベル機能を追加する必要があります。   
このコードを追加することで文字も含めたボタンの実装が完了となります！
...
以上でこのレシピは終了です！   
このレシピではAIモデルの作成やデータの送受信、デスクトップアプリの作成まで幅広い内容を含んでいます。一つ一つの機能だけでも非常に利用する機会が多いと思いますので、
